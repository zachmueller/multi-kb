...
**Note:** This command assumes complete task breakdown exists in tasks.md. Run after `/speckit-05-tasks` completion.
...
**Abort Conditions:** If any required file is missing, instruct user to run prerequisite commands:

- Missing `spec.md` → Run `/speckit-01-specify`
- Missing `plan.md` → Run `/speckit-03-plan`
- Missing `tasks.md` → Run `/speckit-05-tasks`
...
Follow the commit standards defined in `.claude/rules/git.md` to commit any fixes or updates made based on the analysis findings.
...
- Completed specification (`/speckit-01-specify`)
- Implementation plan (`/speckit-03-plan`)
- Task breakdown (`/speckit-05-tasks`)
