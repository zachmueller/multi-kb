...
Run this command with a feature description to:
...
## Notes
Items marked incomplete require spec updates before `/speckit-02-clarify` or `/speckit-03-plan` commands.
```
...
Follow the commit standards defined in `.claude/rules/git.md` to commit the created specification files. The commit should include all files in the `specs/{feature-name}/` directory.
...
- Local-first operation unless network calls are clearly needed
...
- Run `/speckit-02-clarify` if clarification needed
