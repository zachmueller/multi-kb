...
**Note:** This command should run BEFORE `/speckit-03-plan`. Skipping clarification increases downstream rework risk.
...
- If Outstanding/Deferred remain → Assess if should proceed to `/speckit-03-plan` or run `/speckit-02-clarify` again
...
Follow the commit standards defined in `.claude/rules/git.md` to commit the updated specification file(s).
...
- Run `/speckit-03-plan` to create implementation plan
- Consider re-running `/speckit-02-clarify` if high-impact items remain Outstanding

## Error Handling

**Missing Spec File:** If no spec.md found, instruct user to run `/speckit-01-specify` first.
